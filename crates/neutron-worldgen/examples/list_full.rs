...
fn main() {
        ...
        for lx in 0..32i32 {
          if let Ok(Some(data)) = region.get_chunk(lx,lz) {
            if let Ok(nbt) = read_nbt(&data) {
              ...
              let cx = rx*32+lx; let cz = rz*32+lz;
              by_status.entry(status).or_default().push((cx,cz,data.len()));
            }
          }
        }
      }
  ...
  for (st, mut v) in by_status {
    ...
    println!("{st}: count={} largest={} at ({},{})", v.len(), v[0].2, v[0].0, v[0].1);
    if st.contains("full") || st.contains("light") || st.contains("spawn") || st.contains("carver") {
