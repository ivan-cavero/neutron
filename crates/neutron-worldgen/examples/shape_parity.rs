...
fn solid_class(name: &str) -> u8 {
    ...
    if n == "air" || n == "cave_air" || n == "void_air" { return 0; }
    if n == "water" || n == "lava" { return 1; }
    if n.contains("leaves") || n.contains("log") || n == "leaf_litter" || n.contains("sculk") 
        || n == "short_grass" || n == "vine" || n == "glow_lichen" { return 2; } // veg
    3 // solid terrain
}
...
fn main() {
    ...
    for sec in sections {
        ...
        let get = |i: u32| -> String {
            if nstates == 1 { return names[0].clone(); }
        ...
        for i in 0..4096u32 {
            ...
            if idx < van.len() { van[idx] = solid_class(&get(i)); }
        }
    }
    let gen = ChunkGenerator::new(12345);
    let chunk = gen.generate_chunk(6, -2);
    let mut match_sa = 0u32; let mut tot_sa = 0u32;
    let mut match_solid = 0u32; let mut tot_solid = 0u32;
        ...
        for z in 0..16usize {
            for x in 0..16usize {
                ...
                if vs == 0 || vs == 1 || vs == 3 {
                    ...
                    let vbin = if vs==0 {0} else if vs==1 {1} else {2};
                    let nbin = if ns==0 {0} else if ns==1 {1} else {2};
                    if vbin==nbin { match_sa += 1; }
                }
                if vs==3 && ns==3 { match_solid += 1; }
                if vs==3 || ns==3 { tot_solid += 1; }
            }
        }
    }
    println!("air/fluid/solid shape match: {match_sa}/{tot_sa} ({:.2}%)", 100.0*match_sa as f64/tot_sa as f64);
}
