...
use neutron_worldgen::{ChunkGenerator, surface::BlockId, biome_source::{climate_at_block, find_biome, biome_id}, density::DensityEnv, generator::WORLD_BOTTOM, feature_rng::FeatureRandom};
...
fn main() {
    ...
    for _ in 0..256 {
      ...
      let open = matches!(b, BlockId::Air|BlockId::Water|BlockId::Sculk|BlockId::SculkVein|BlockId::SculkCatalyst);
      if !open {
      ...
      for (dx,dy,dz) in dirs {
        let nx = lx as i32 + dx; let nz = lz as i32 + dz; let ny = y + dy;
        if nx<0||nx>=16||nz<0||nz>=16||ny<WORLD_BOTTOM||ny>=320 { continue; }
        let nb = ch.block_at(nx as u32, ny, nz as u32);
        if !matches!(nb, BlockId::Air|BlockId::Water|BlockId::Lava|BlockId::Sculk|BlockId::SculkVein|BlockId::SculkCatalyst|BlockId::OakLeaves|BlockId::Snow|BlockId::PowderSnow) {
          has=true; break;
        }
      }
      if has { would_spread+=1; *reasons.entry("spread_ok".into()).or_default() += 1; }
      else { *reasons.entry("open_no_solid_n".into()).or_default() += 1; }
    }
    println!("biome_ok={biome_ok} would_spread={would_spread}");
    let mut v: Vec<_> = reasons.into_iter().collect();
    v.sort_by(|a,b| b.1.cmp(&a.1));
    for (k,c) in v { println!("  {c:4} {k}"); }
}
