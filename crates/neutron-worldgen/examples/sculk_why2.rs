...
fn main() {
    ...
    let mut biome_ok=0u32; let mut spread=0u32; let mut on_vein=0u32; let mut on_air=0u32;
    for _ in 0..256 {
      ...
      let sculk_beh = matches!(b, BlockId::Sculk|BlockId::SculkVein|BlockId::SculkCatalyst);
      let open = matches!(b, BlockId::Air|BlockId::Water);
      if sculk_beh { on_vein+=1; spread+=1; continue; }
      if !open { continue; }
      on_air+=1;
    ...
    let mut open_dd=0u32; let mut vein_dd=0u32; let mut sculk_dd=0u32;
    for y in WORLD_BOTTOM..320 {
      for z in 0..16u32 { for x in 0..16u32 {
        let wx=ox+x as i32; let wz=oz+z as i32;
    ...
    println!("patch samples: biome_ok={biome_ok} spread={spread} on_vein={on_vein} on_air_open={on_air}");
