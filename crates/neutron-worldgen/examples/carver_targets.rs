...
fn main() {
    ...
    for (tcx, tcz) in [(6,-2),(5,-2),(6,-1),(7,-2),(6,-3),(0,0),(10,10)] {
        ...
        for y in WORLD_BOTTOM..320 {
            for z in 0..16 {
                for x in 0..16 {
                    if matches!(region.get(tcx*16+x,y,tcz*16+z), BlockId::Air|BlockId::Lava) { air+=1; }
                }
            }
        }
        println!("target({tcx},{tcz}): starts={} writes={} air={}",
            CARVE_STARTS.load(Ordering::Relaxed),
            CARVE_WRITES.load(Ordering::Relaxed), air);
    }
    ...
    for index in 0..3i64 {
        ...
        println!("local index={index} nextFloat={f:.6} prob={p} start={}", f<=p);
        if f<=p { local_starts+=1; }
    }
