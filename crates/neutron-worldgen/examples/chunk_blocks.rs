...
fn main() {
    ...
    for sec in sections {
        ...
        if names.len()==1 {
            *hist.entry(names[0].clone()).or_default() += 4096;
            if y_sec <= 0 { *deep_hist.entry(names[0].clone()).or_default() += 4096; }
        } else {
            ...
            for i in 0..4096u32 {
                ...
                *hist.entry(n.clone()).or_default() += 1;
                if y_sec <= 0 { *deep_hist.entry(n).or_default() += 1; }
            }
        }
    }
    ...
    for (n,c) in &v {
        if n.contains("sculk") || n.contains("deepslate_brick") || n.contains("deepslate_tile")
            || n.contains("wool") || n.contains("candle") || n.contains("soul")
            || n.contains("chest") || n.contains("ancient") || n.contains("reinforced")
            || n.contains("mineshaft") || n.contains("plank") || n.contains("cobweb")
            || n.contains("rail") || n.contains("fence") {
            println!("  {c:5} {n}");
        }
    }
