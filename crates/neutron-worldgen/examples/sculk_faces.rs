...
fn main() {
  ...
  let mut floor=0u32; let mut ceil=0u32; let mut wall=0u32; let mut multi=0u32;
  let mut wrong_floor=0u32; let mut wrong_ceil=0u32; let mut wrong_wall=0u32; let mut wrong_multi=0u32;
  let mut neu_sculk=0u32; let mut match_c=0u32; let mut over=0u32;
  for y in WORLD_BOTTOM..320 {
    for z in 0..16i32 {
      for x in 0..16i32 {
        ...
        // open dirs
        let up = matches!(ch.block_at(x as u32,y+1,z as u32), BlockId::Air|BlockId::Water|BlockId::SculkVein);
        let dn = y>WORLD_BOTTOM && matches!(ch.block_at(x as u32,y-1,z as u32), BlockId::Air|BlockId::Water|BlockId::SculkVein);
        let mut side=false;
        for (dx,dz) in [(-1,0),(1,0),(0,-1),(0,1)] {
          let nx=x+dx; let nz=z+dz;
          if nx<0||nx>=16||nz<0||nz>=16 { continue; }
          if matches!(ch.block_at(nx as u32,y,nz as u32), BlockId::Air|BlockId::Water|BlockId::SculkVein) { side=true; }
        }
        let nopen = (up as u32)+(dn as u32)+(side as u32);
        if van_sc {
          match_c+=1;
          if up && !dn && !side { floor+=1; }
          else if dn && !up && !side { ceil+=1; }
          else if side && !up && !dn { wall+=1; }
          else { multi+=1; }
        } else {
          over+=1;
          if up && !dn && !side { wrong_floor+=1; }
          else if dn && !up && !side { wrong_ceil+=1; }
          else if side && !up && !dn { wrong_wall+=1; }
          else { wrong_multi+=1; }
        }
        let _ = nopen;
      }
