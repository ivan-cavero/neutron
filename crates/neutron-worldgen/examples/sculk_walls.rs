use neutron_worldgen::{ChunkGenerator, surface::BlockId, biome_source::{climate_at_block, find_biome, biome_id}, density::DensityEnv, generator::WORLD_BOTTOM};
use neutron_world::nbt::ussr_nbt::owned::{List, Tag};
use neutron_world::nbt::{compound_get, read_nbt};
use neutron_world::Region;
use std::path::PathBuf;
fn is_solid(b: BlockId) -> bool {
  !matches!(b, BlockId::Air|BlockId::Water|BlockId::Lava|BlockId::Sculk|BlockId::SculkVein|BlockId::SculkCatalyst|BlockId::ShortGrass|BlockId::OakLeaves|BlockId::Snow|BlockId::PowderSnow)
}
fn main() {
    ...
    for z in 0..16i32 {
      for x in 0..16i32 {
        ...
        for (dx,dy,dz) in dirs {
          ...
          if matches!(nb, BlockId::Air|BlockId::Water|BlockId::SculkVein) { has_open=true; break; }
        }
        if has_open {
          wall+=1;
          if matches!(b, BlockId::Sculk|BlockId::SculkCatalyst) { sculk_wall+=1; }
        }
      }
    }
  }
  ...
  for sec in sections {
    ...
    for i in 0..4096u32 {
      ...
      if name.contains("sculk") && !name.contains("vein") { // sculk + catalyst + sensor etc
        if name.ends_with(":sculk") || name.contains("catalyst") { van_sculk+=1; }
      }
    }
    let _ = y_sec;
  }
  ...
  for sec in sections {
    ...
    for i in 0..4096u32 {
      ...
      if idx < van_name.len() { van_name[idx]=name; }
    }
  }
  for n in &van_name {
    let s=n.strip_prefix("minecraft:").unwrap_or(n);
    if s=="sculk" || s=="sculk_catalyst" { van_sculk2+=1; }
  }
  // overlap: van sculk where neu is solid wall
  let mut match_pos=0u32; let mut van_only=0u32;
  for y in WORLD_BOTTOM..320 {
    for z in 0..16usize {
      for x in 0..16usize {
        ...
        if matches!(nb, BlockId::Sculk|BlockId::SculkCatalyst) { match_pos+=1; }
        else { van_only+=1; }
      }
    }
  }
