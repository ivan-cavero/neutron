...
pub fn simulate_canonical_pregen() -> Vec<(i32, i32)> {
    // s81: NEUTRON_TRUE_ORDER=1 replays the TRUE decorate order captured from
    // a real server boot (see sculk::decoration_origin_order). Gated behind
    // the env so the default sim (and its tests) stay deterministic.
...
/// The set of chunks the canonical ref procedure actually decorated. Chunks
/// outside this footprint were never saved/generated by the ref server; any
/// decoration pass attributed to them would write trees/blocks the real world
/// never had (their `window_order` rank trails at u64::MAX).
pub fn decorate_footprint() -> &'static std::collections::HashSet<(i32, i32)> {
    static SET: OnceLock<std::collections::HashSet<(i32, i32)>> = OnceLock::new();
    SET.get_or_init(|| decorate_sequence().iter().copied().collect())
}
