...
//
// ref-extract: Extract chunk checksums from a Minecraft server for parity verification.
//
...
#[derive(Parser, Debug)]
#[command(name = "ref-extract", version, about)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    ExtractAll {
        ...
        /// Output directory (default: tools/ref-extract/hashes).
        #[arg(long)]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        None => {
            // Default: show help
            Cli::parse_from(["ref-extract", "--help"]);
            Ok(())
        }
    }
}
...
fn cmd_extract(
    ...
    tracing::info!(
        ...
        "starting ref-extract extraction"
    );
    ...
    let tmp_dir = match &tmp_dir {
        ...
        None => {
            let base = std::env::temp_dir().join(format!("ref-extract-{}", seed));
            if base.exists() {
...
fn cmd_extract_all(
    ...
    let servers_dir = servers_dir.unwrap_or_else(|| repo_root.join("bench").join("servers"));
    let output_dir =
        output_dir.unwrap_or_else(|| repo_root.join("tools").join("ref-extract").join("hashes"));
