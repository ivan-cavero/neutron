...
//
// neutron-hash: Compare Neutron-generated chunks against vanilla reference data.
//
// Two hash modes:
...
// compared against a vanilla reference JSON with `ref-extract compare`.
...
/// JSON comparable against a vanilla reference via `ref-extract compare`.
#[derive(Parser, Debug)]
#[command(name = "neutron-hash", version, about)]
struct Cli {
    /// World seed.
    #[arg(long)]
    seed: i64,
...
fn main() -> Result<()> {
    ...
    if let Some(output_path) = &cli.generate_neutron {
        ...
        println!(
            "Compare against a vanilla reference with: cargo run -p ref-extract -- compare \\
...
fn serialize_chunk_to_nbt_bytes(chunk: &neutron_worldgen::GeneratedChunk) -> Vec<u8> {
    ...
    // Wrap in a compound and serialize (matching ref-extract's hash format)
