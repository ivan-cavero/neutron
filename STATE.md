...
Worldgen 1:1 vs vanilla **26.2**. HEAD pending: waterlogged vegetation patch + `nextBoolean` selector.
...
- clay full `(0,0)`: Neutron 537 vs vanilla 509 (`clay_overlap` xz overlap 96/134).
- water y[0,16): Neutron **(0,0)=4 (0,1)=111** vs vanilla **15 + 97**. Probe cells **14/22 water**.
...
424242 region **97.71%** (+0.37pp). Water in `(0,1)` y0..16 is in range (111 vs 97); `(0,0)` still short (4 vs 15). 8/22 probe cells clay/moss (`(12,1,15)` and other (0,0) floors; `(0,5,24)`/`(1,5,25)` had solid above in doFill).

Next dump: those 8 cells vs vanilla `--list` (same seed/coords) — exposed-edge clay vs dry `clay_with_dripleaves` vs `isFaceSturdy`. Do not reopen springs/carvers.

## Dead (do not reopen without a new two-sided dump)

Chunk decoration order · cave-biome stored-grid vs voronoi · `vegetation_patch` HashSet · noodle sign (compared seed 12345 vs 424242) · `getInterpolatedNoiseValue` helper as if it were `doFill` · T4 feature ports as the 424242 recall lever · classic carvers as the writer of 424242 `(0,0)`/`(0,1)` y[-16,16) water · worm start-Y desync · `SpringFeature` as the writer of those floor cells · `nextInt(2)` as `RandomBooleanSelectorFeature` pick.
