# STATE — Neutron

> Estado actual del proyecto. Se lee al empezar cada run y se actualiza al terminar.

- **Fase actual**: F0 — Fundamentos y harness (bar: baseline B0 reproducible por un agente distinto; 10 bots join p95 < 5 s; cps ±30% vs baselines publicados).
- **Último run**: 000 — Fundación (terminado, 5 ago 2026).
- **Siguiente run**: 001 — F0: harness de benchmarks + baseline B0.
- **Herramienta**: opencode (un agente = LEAD + builder; critic como subagente con contexto limpio).
- **Carpeta de trabajo (regla)**: TODO se escribe en la carpeta del proyecto — el directorio de trabajo actual desde el que se ejecuta todo (AGENTS.md §0). Prohibido worktrees/carpetas externas.
- **Presupuesto acumulado**: N/A (solo documentación hasta ahora).
- **Historial**: runs/run-*.md · Método: AGENTS.md §2.